...
### 4-1. 백엔드 - 필터 API
- [ ] `GET /api/reminders?filter=today` - 오늘 날짜의 리마인더
- [ ] `GET /api/reminders?filter=scheduled` - remindAt이 설정된 리마인더 (날짜순)
- [ ] `GET /api/reminders?filter=completed` - completed=true 리마인더
...
- [ ] `Subtask` 엔티티 생성 (id, title, completed, reminderId, sortOrder)
- [ ] `SubtaskRepository` / `SubtaskService` / `SubtaskController` 생성
- [ ] API: `GET/POST/PUT/DELETE /api/reminders/{id}/subtasks`
- [ ] 검색 API: `GET /api/reminders?search=keyword` (title, description LIKE 검색)
