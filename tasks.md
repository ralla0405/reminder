...
### 5-1. 사이드바 - 리스트 목록
- [x] 사이드바에 리스트 목록 표시 (원형 컬러 아이콘 + 이름 + 항목 수)
- [x] 리스트 생성 UI (+ 버튼 → 이름/색상 입력)
- [x] 리스트 수정/삭제 UI (우클릭 또는 ... 메뉴)
- [x] 색상 선택 팔레트 (Apple 기본 12색)

### 5-2. 리스트별 리마인더 표시
- [x] 사이드바 리스트 클릭 → 해당 리스트의 리마인더만 메인 영역에 표시
- [x] 리마인더 생성/수정 시 리스트 선택 드롭다운
- [x] 메인 영역 상단 헤더에 리스트 이름 + 색상 표시

### 5-3. 필터 카드 UI
- [x] 사이드바 상단 2x2 그리드 필터 카드 컴포넌트
- [x] 오늘 (파랑 아이콘) / 예정 (빨강) / 전체 (검정) / 완료됨 (회색)
- [x] 각 카드에 숫자 배지 (카운트 API 연동)
- [x] 카드 클릭 시 필터된 리마인더 목록 표시

### 5-4. 예정 뷰 날짜 그룹핑
- [x] 날짜별 섹션 헤더 ("오늘", "내일", "이번 주", "이후")
- [x] 각 섹션 내 리마인더 시간순 정렬

### 5-5. 우선순위 UI
- [x] 상세 패널에 우선순위 선택 UI (없음/낮음/보통/높음)
- [x] 목록에서 우선순위 표시 (느낌표 아이콘: !, !!, !!!)
- [x] 우선순위별 정렬 옵션 추가

### 5-6. 하위 작업 UI
- [x] 상세 패널에 하위 작업 추가/삭제 UI
- [x] 하위 작업 체크박스 완료 토글
- [x] 목록 항목에 하위 작업 진행률 표시 ("2/5" 형태)

### 5-7. 검색
- [x] 사이드바 상단 검색 입력 바
- [x] 실시간 검색 (debounce 300ms)
- [x] 검색 결과에서 매칭 텍스트 하이라이트
